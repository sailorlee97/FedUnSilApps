...
class TrainerPrivate(object):
    ...
    def _local_update(self,dataset, local_ep, lr,optim_choice, ul_mode_train='none'):
        ...
        train_ldr  = DataLoader(BatchflowData(train_x, train_y),
                                batch_size=256, shuffle=True, drop_last=True)
        # for _, labels in dataloader:
        #     all_labels.extend(labels.numpy().tolist())

        unique_classes = set(all_labels)  # 使用集合统计唯一标签
        num_classes = len(unique_classes)

        #print(f"类别数量：{num_classes}")
        #print(f"类别标签：{unique_classes}")
        ...
        for epoch in range(local_ep):
            ...
            for batch_idx, (x, y) in enumerate(train_ldr):
                ...
                elif 'amnesiac' in ul_mode_train:
    ...
    def _local_update_ul(self,dataset, local_ep, lr, optim_choice, ul_class_id, ul_mode_train=None):
        ...
        for epoch in range(local_ep):
            ...
            for batch_idx, (x, y) in enumerate(train_ldr):
                ...
                if 'ul_samples' in self.ul_mode:   #self.ul_mode=='ul_samples' or self.ul_mode=='ul_samples_backdoor' or 'u_samples_whole_client: # random false labels
                    ...
                    # f_label=random.randint(0,9)
                elif self.ul_mode=='fedau':
                    #ul_class_id = [0,1]
                    one_hot_labels=[]
                    for label in y:
                        label = label.long()
                        one_hot_label_ul=torch.nn.functional.one_hot(torch.tensor(ul_class_id), self.num_classes).to(self.device, dtype=torch.int64)
    ...
    def test(self, dataloader):
        ...
        with torch.no_grad():
            for x,y in dataloader:
                ...
                #target = target.long()
                pred = self.model(x)  # test = 4
                # print(target)
                loss_meter += F.cross_entropy(pred, y, reduction='sum').item() #sum up batch loss
    ...
    def test_new(self, dataloader):
        ...
        with torch.no_grad():
            for x, y in dataloader:
                ...
                x = x.unsqueeze(1).unsqueeze(-1)
                #x, y = x.to(self.device), y.to(self.device)
                #x = x.unsqueeze(1).unsqueeze(-1)  # 适配模型的输入形状

                # 模型预测
                pred = self.model(x)  # 输出形状 [batch_size, num_classes]
                pred = pred.argmax(dim=1)  # 获取每个样本的预测类别
